...
- **Slice 4 (synthesis):** cluster via the Slice 2 embeddings → keep the highest-composite exemplar per cluster → write `WORKFLOW_STRATEGY.md` grouped by verdict bucket: **Build / integrate now (INVEST — flagged for Timothy's confirmation, not auto-committed)** → **Adopt cheaply, watch for the product (TRACK)** → **Wait for the model (WAIT)**. DISCARD/ARCHIVE excluded. The bucket is the section structure of the doc.
