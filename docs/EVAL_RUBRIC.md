...
- **source_credibility (0–3)** — `0` = SEO farm / engagement-bait; `1` = solo thread, no receipts; `2` = practitioner/eng blog with detail; `3` = lab / primary / peer-reviewed / first-party. Partly a per-source attribute (see `sources.yaml`), partly per-item. *(See Reviewer note 4.)*
...
- **commoditization_risk (0–3)** — will the tooling ship this as a built-in? `0` = no vendor would productize it for you; `2` = plausibly on a roadmap; `3` = obviously about to be a feature (don't build infra around it). *(See Reviewer note 2.)*
...
5. **INVEST** — else if `composition_specificity ≥ 2` (you reach here only if durable + low-commoditization). Build/integrate into the stack; this compounds and won't be commoditized. Top of `WORKFLOW_STRATEGY.md`. *(See Reviewer note 1.)*
...
- **Slice 3 (triage):** the cheap model emits Zod-validated JSON per item — the eight scores, `hype_markers[]`, the derived verdict bucket, a one-line rationale, and extracted claim + evidence snippets. Stored on the `items` row in dedicated columns (telemetry-in-columns convention) so re-ranking never re-calls the model. *(See Reviewer note 6.)*
- **Slice 4 (synthesis):** cluster via the Slice 2 embeddings → keep the highest-composite exemplar per cluster → write `WORKFLOW_STRATEGY.md` grouped by verdict bucket: **Build / integrate now (INVEST)** → **Adopt cheaply, watch for the product (TRACK)** → **Wait for the model (WAIT)**. DISCARD/ARCHIVE excluded. The bucket is the section structure of the doc.
...
*Added in review. The original invited pushback on two load-bearing decisions (DISCARD hard-drop vs. quarantine; the `commoditization_risk ≥ 2` cutoff); this section answers both and adds refinements. Core verdict: **adopt** — these are tightening, not rejection.*

1. **INVEST rests on the two softest cheap-model dimensions.** Both gates for INVEST — `commoditization_risk` (roadmap prediction) and `composition_specificity` (fit to a specific stack) — are precisely what a cheap model judges worst, and INVEST is the most consequential verdict (it spends real build effort). **Treat INVEST as a human-review candidate**: the synthesis surfaces INVEST items for confirmation rather than auto-committing them. Auto-routing is fine for the lower-stakes buckets (DISCARD / ARCHIVE / WAIT / ADOPT-CHEAP).

2. **`commoditization_risk ≥ 2` must cite a concrete signal** (the flagged load-bearing cutoff). A cheap model free-guesses at vendor roadmaps. Gate a high score behind an explicit cited signal — a vendor announcement, a beta, a roadmap item — in the rationale; absent that, cap it at 1. This ties the most-speculative dimension to evidence and is the main mis-routing guard, more so than the exact numeric cutoff.
...
4. **Move the `source_credibility` baseline into `sources.yaml`.** Add a per-source `source_tier` (lab = 3, practitioner = 2, …) so the model isn't re-deriving credibility for every item from the same source. The model only adjusts per-item from that baseline — more consistent, less model load.
...
**Sequencing:** this is the Slice 3–4 spec. **Slice 2 (dedup) is the next build** — Slice 4's clustering depends on its embeddings. The refinements above are decisions to finalize before Slice 3, not now.
